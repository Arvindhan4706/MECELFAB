    ...
    <div className="min-h-screen bg-black pt-32 pb-24">
      <div className="container mx-auto px-4 sm:px-6 md:px-8 max-w-6xl">
        <div className="max-w-3xl mb-20">
          ...
          <div className="h-12 md:h-16 w-1/2 bg-white/5 rounded mb-4 animate-pulse" />
          ...
          {Array.from({ length: 2 }).map((_, i) => (
            <div key={i} className="p-8 border border-white/5 bg-white/[0.02]">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 border border-white/10 rounded-sm animate-pulse" />
                <div className="h-6 w-40 bg-white/5 rounded animate-pulse" />
              </div>
              <div className="h-4 w-full bg-white/5 rounded mb-6 animate-pulse" />
              <div className="flex flex-col gap-3">
                {Array.from({ length: 4 }).map((_, j) => (
                  <div key={j} className="h-3 w-48 bg-white/5 rounded animate-pulse" />
                ))}
              </div>
            </div>
          ))}
